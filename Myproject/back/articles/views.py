...
def comment_create(request, pk): #댓글 생성 => 어떤 게시글에 있는 댓글인지에 대한 내용 : pk
    ...
    data['user'] = request.user.id  # 현재 로그인된 사용자 ID를 할당
    serializer = CommentSerializer(data=data) #data 정보에 POST로 전달한 정보 담음
    if serializer.is_valid(raise_exception=True):
        serializer.save(article=article, user=request.user) # 어떠한 게시글에 대한 내용인지에 대한 내용 전달 => article 
        return Response(serializer.data, status=status.HTTP_201_CREATED)
